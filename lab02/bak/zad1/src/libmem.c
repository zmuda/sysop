...
Diagnostics* diagnose(){
    ...
    Lista* tmp =biggestPredecesor(0);
    if(tmp){
        diags->biggestFree=tmp->next->blocksNum*BLOCK_SIZE;
    } else {
        diags->biggestFree=0;
    }
    tmp= smallestPredecessor(0);
    if(tmp){
        diags->smallestFree=tmp->next->blocksNum*BLOCK_SIZE;
    } else {
        diags->smallestFree=0;
    }
    return diags;
};
/*
